import { ClassTransformer } from "./ClassTransformer.ts";
...
import { TransformMetadata } from "./metadata/TransformMetadata.ts";
import { ClassTransformOptions } from "./ClassTransformOptions.ts";
import { TransformationType } from "./TransformOperationExecutor.ts";
...
export function Transform(
  ...
  return function (target: any, key: string) {
    const metadata = new TransformMetadata(
      target.constructor,
      key,
      transformFn,
      options as any,
...
export function TransformPlainToClass(
  ...
  return function (
    ...
    descriptor.value = function (...args: any[]) {
      ...
      return isPromise
        ? result.then((data: any) =>
          classTransformer.plainToClass(classType, data, params)
        )
