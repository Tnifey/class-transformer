...
export class TransformOperationExecutor {
  ...
  transform(
    ...
    if (Array.isArray(value) || value instanceof Set) {
      ...
      testForBuffer() &&
      (targetType === Deno.Buffer || value instanceof Deno.Buffer) &&
      !isMap
    ) {
      if (value === null || value === undefined) return value;
      ...
      for (let key of keys) {
        ...
        if (!this.options.ignoreDecorators && targetType) {
          if (this.transformationType === TransformationType.PLAIN_TO_CLASS) {
            const exposeMetadata = defaultMetadataStorage
              ...
              if (!(value[valueKey] instanceof Array)) {
                if (
                  ...
                  type = metadata.options.discriminator.subTypes.find(
                    (subType) => {
                      const has = (metadata?.options
                        ?.discriminator as Discriminator)?.property as
                          | string
                          | number;
                      if (subValue && has in subValue) {
                        return (
                          subType.name ===
                            subValue[
                              (metadata?.options
        ...
        if (!this.options.enableCircularCheck || !this.isCircular(subValue)) {
          ...
          if (this.transformationType === TransformationType.CLASS_TO_PLAIN) {
            ...
            finalValue = value[transformKey] === finalValue
              ? subValue
              : finalValue;
  ...
  private applyCustomTransformations(
    ...
    if (this.options.version !== undefined) {
      metadatas = metadatas.filter((metadata) => {
        ...
        return this.checkVersion(
          metadata?.options?.since as number,
          metadata?.options?.until as number,
        );
      });
    }
    ...
    if (this.options.groups && this.options.groups.length) {
      metadatas = metadatas.filter((metadata) => {
        ...
        return this.checkGroups(metadata?.options?.groups as string[]);
      });
    } else {
      metadatas = metadatas.filter((metadata) => {
        return (
    ...
    if (!this.options.ignoreDecorators && target) {
      ...
      if (this.options.version !== undefined) {
        keys = keys.filter((key) => {
          ...
          return this.checkVersion(
            exposeMetadata?.options?.since as number,
            exposeMetadata?.options?.until as number,
          );
        });
      }
      ...
      if (this.options.groups && this.options.groups.length) {
        keys = keys.filter((key) => {
          ...
          return this.checkGroups(exposeMetadata?.options?.groups as string[]);
        });
      } else {
        keys = keys.filter((key) => {
          const exposeMetadata = defaultMetadataStorage.findExposeMetadata(
  ...
  private checkVersion(since: number, until: number) {
    let decision = true;
    if (decision && since) {
      decision = (this?.options?.version as number) >= since;
    }
    if (decision && until) {
      decision = (this?.options?.version as number) < until;
    }
  ...
  private checkGroups(groups: string[]) {
    ...
    return this.options?.groups?.some(
      (optionGroup) => groups.indexOf(optionGroup) !== -1,
    );
  }
}

function instantiateArrayType(arrayType: Function): Array<any> | Set<any> {
  const array = new (arrayType as FunctionConstructor)();
